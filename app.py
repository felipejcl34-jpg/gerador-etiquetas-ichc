...
# Lógica de Edição: Recupera valores se estiver editando
...
val_soro_total = st.session_state.fila_etiquetas[idx].get("soro_total", "") if idx is not None else ""
...
if is_soro:
    st.info("💡 Campos de Soro ativados (Opcionais)")
    cs1, cs2 = st.columns([2, 1])
    with cs1: soro_comp = st.text_area("Aditivos / Composição", value=val_soro_comp, placeholder="Ex: cloreto de SODIO 20% 40ml\ncloreto de POTASSIO 19,1% 10ml")
    with cs2: soro_total = st.text_input("Total a ser infundido", value=val_soro_total, placeholder="Ex: 1050ml")
...
if st.session_state.fila_etiquetas:
    ...
    for i, e in enumerate(st.session_state.fila_etiquetas):
        ...
        with col_txt:
            info_soro = f" | Total: {e['soro_total']}" if e.get('soro_total') else ""
            st.write(f"**{i+1}.** {e['med']} | {e['dose']} | {e['via']} | **Hs: {e['hora']}**{info_soro}")
        with col_ed:
    ...
    for item in st.session_state.fila_etiquetas:
        ...
        if item.get("soro_comp"):
            ...
            # Total Infundido
            if item.get("soro_total"):
                c.setFont("Helvetica-Bold", 7)
                c.drawString(2*mm, 5*mm, f"TOTAL: {item['soro_total']}")
        else:
        ...
        # Horário (Sempre no canto inferior direito)
